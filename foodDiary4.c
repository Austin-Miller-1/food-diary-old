...
void upload_data(char* upc, char* imageName, double mass, char* albumId);
...
int main(void)
{
  ...
  */
  ...
  for(;;){
    ...
    double newMass = -1; //Give impossible first value
    while(newMass != mass){
        mass = newMass;
    ...
    if(newMass >= mass){
      ...
      if(photoTaken){
        ...
        upload_data(upc, imageName, token, newMass, albumId);
      ...
      take_picture(imageName, cameraFlags);
      upload_data(NULL, imageName, token, newMass, albumId);
      }
...
int get_imgur_token(char* token, char* albumId, char* username){
  int resultBufSize = MAX_TOKEN_SIZE+100;
  ...
  sprintf(commandToRun, 150, "echo $(http GET %s/api/getImgurToken 'username=%s') >> %s", WEBSITE, username, IMGUR_TOKEN_FILE_NAME);
  system(commandToRun);
  ...
  int albumIdStart; int albumIdEnd; int albumIdSize;
  for(; c < resultBufSize; c++){
    if(resultBufSize[c] == ':'){
      albumIdStart = (c+=2); //Start of token is therefore 2 characters ahead ( "field":"token" )
      while(resultBuffer[++c] != '\"'){;}  //Keep going until the end quote is found
      albumIdEnd = c-1; //End token
    }
  }
...
int process_barcode(char* upc, char* imageName){
  ...
  if(!resultBuffer.strcmp("NULL")){printf("No UPC detected"); return -1;}
  else{
  ...
  //If food...
    ...
    if( upc == NULL){
      ...
      */
      ...
      snprintf(header, MAX_TOKEN_SIZE+100, "Authorization: Bearer %s\0", token);
    ...
    //  snprintf(serverJSON, SERVER_BUFFER_SIZE, "mode=food mass=", mass);
      ...
      sprintf(commandToRun, 150, "echo $(http GET %s/api/sendData 'imgurid:%s' 'mode:food' 'mass:%s') >> %s", WEBSITE, mass);
      ...
      sprintf(commandToRun, 150, "echo $(http GET %s/api/sendData 'upc:%s' 'mode:barcode' 'mass:%s') >> %s", upc, WEBSITE, mass);
...
void create_JSON(char* buffer, char* imageName, int isBarcode, double mass){
  ...
  //If barcode, downscale image, scan it for barcode, then send data to imgur
  if(isBarcode){
    //char
    /*
    //New image name
    sprintf(newImageName, 100, "small-%s", imageName);

    //Downscale image to usable size
    char commandToRun[1000];
    sprintf(commangToRun, 1000, "convert %s -resize 1224x919 %s", imageName, newImageName);
    system(commandToRun);
    */

    /*
    Get barcode
    get_barcode(newImageName);
  ...
  //Send request to server to get imgur authentification key, save response to 'response' file
  commandToRun[0] = '\0'; //Reset command buffer
  sprintf(commangToRun, 1000, "http PUT %s/api/getImgurToken field=val > response", WEBSITE);
  system(commandToRun);
  */
  ...
  snprintf(newImageName, 100, "e%s", imageName);
  snprintf(commandToRun, 1000, "./base64 -e %s %s", imageName, newImageName);
  ...
  if(bytesRead == 0){perror("could not read file");}
  base64[bytesRead] = '\0';
  snprintf(buffer, 5000,"{\"imageName\":\"%s\",\"mode\":\"%s\",\"mass\":\"%f\",\"imageData\":\"data:image/jpeg;base64,%s\"}", newImageName, mode, mass, base64);


  /*
    //Authenticate with imgur api

    //Make json file for imgur
    snprintf(buffer, 5000,"{\"image\":\"%s\",\"title"\:\"%s\",}\0", base64, imageName);

    //Make header for imgur
    char header[100];
    snprintf(header, 100, "Authorization: Bearer %s\0", imgurAccessToken);

    //Send request to imgur API & save response
    commandToRun[0] = '\0'; //Reset command buffer
    snprintf(commangToRun, 1000, "curl -X POST -H '%s' -d '%s' %s", header, buffer, IMGUR_API) > imgurResponse;
    system(commandToRun);
  */
