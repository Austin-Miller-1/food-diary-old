...
/*
  ...
  */
  ...
  sprintf(commangToRun, 1000, "convert %s -resize 1224x919 small-%s", imageName, imageName);
  system(commandToRun);
  */
  ...
  sprintf(commangToRun, 1000, "http PUT %s/api/test-session field=val > response", WEBSITE);
  system(commandToRun);
  */
  ...
  /*
    ...
    commandToRun[0] = '\0'; //Reset command buffer
    sprintf(commangToRun, 1000, "curl -X POST -H 'Content-Type: application/json' -d '%s' %s", buffer, IMGUR_API) > imgurResponse;
    system(commandToRun);
  */
